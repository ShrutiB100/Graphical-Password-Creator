...
This project will change the way people protect their digital identities. It introduces a new method of creating passwords through the use of a graphical interface. The password is saved by the user in the form of coordinates.
The first layer of security is selecting a picture, which is followed by the username, which is an additional layer of security that is unique to each individual. The fundamental data is the entered coordinates; this data is stored in mysql, which is known to be a very secure relational database. MySQL includes a strong data security layer that shields sensitive data from intruders. Database security measures, such as SQL injection, can help protect against common attacks that can circumvent access controls. It can also protect your data from compromise, which can result in reputational harm, data loss, and consumer distrust.
...
*   Hard to guess the password according to the user’s background.
*  The security system is very high.
